...
<template>
  <a
    ...
    class="inline-flex items-center gap-3 rounded-2xl px-4 py-2.5 transition-all duration-200 hover:-translate-y-0.5 select-none"
    :class="variant === 'white'
  ...
  >
    <!-- Google Play icon -->
    <svg v-if="store === 'google'" class="w-8 h-8 shrink-0" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <!-- Apple icon -->
    <svg v-else class="w-8 h-8 shrink-0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" :fill="variant === 'white' ? '#000' : '#fff'">
    ...
    <div class="leading-tight">
      <template v-if="store === 'google'">
        <span class="block text-[10px] uppercase tracking-wider" :class="variant === 'white' ? 'text-ink-500' : 'text-white/70'">Disponible en</span>
